...
class InlineKeyboardPaginator:
    ...
    def __init__(self, page_count, current_page=1, data_pattern='{page}'):
        self._keyboard_before = list() # NOQA[C408]
        self._keyboard_after = list() # NOQA[C408]
    ...
    def _build(self):
        keyboard_dict = dict() # NOQA[C408]

        if self.page_count == 1:
            self._keyboard = list() # NOQA[C408]
            return
